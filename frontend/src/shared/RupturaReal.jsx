...
export function VisaoGeralReal({ municipio, onNavigate, estadual = false }) {
  ...
  const cards = kpi ? [
    ...
    { label: 'Índice de risco regional', value: decimal(kpi.indice_risco_regional), detail: `Escala de 0 a 100 · ${variacao(kpi.variacao_indice_risco_pp, ' pontos', periodoLabel)}`, key: 'indice_risco_regional', color: 'var(--risk-alto)' },
    { label: estadual ? 'Municípios em alerta de suprimento' : 'Em alerta de suprimento', value: estadual ? inteiro(kpi.municipios_alerta_suprimento) : (n(kpi.municipios_alerta_suprimento) == null ? 'Não informado' : n(kpi.municipios_alerta_suprimento) > 0 ? 'Sim' : 'Não'), detail: estadual ? 'Contagem estadual, sem variação percentual' : 'Indicador municipal, sem variação percentual', key: 'municipios_alerta_suprimento', color: 'var(--risk-medio)' },
  ...
  return <div className="rise">
    ...
    <Card className="p-5" style={{ marginTop: 18 }}><SectionTitle>Evolução de casos</SectionTitle><p style={texto}>Histórico SINAN e tendência estimada. O gráfico não muda com o comparativo dos cards.</p>{evolucao.length ? <ResponsiveContainer width="100%" height={300}><ComposedChart data={evolucao}><CartesianGrid stroke="var(--ink-100)" vertical={false} /><XAxis dataKey="competencia" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Legend verticalAlign="top" height={28} iconType="plainline" wrapperStyle={{ fontSize: 11.5 }} /><Area dataKey="casos_notificados" name="Casos notificados" fill="var(--primary)" fillOpacity={0.08} stroke="var(--primary)" /><Line dataKey="casos_tendencia" name="Tendência estimada" stroke="var(--ink-400)" strokeDasharray="6 4" dot={false} /></ComposedChart></ResponsiveContainer> : <Vazio texto="Série de evolução com tendência não disponível para este território." />}</Card>
...
export function InsumosReais({ municipio }) {
  ...
  return <div className="rise">
    ...
    {resumo ? <div className="responsive-grid-4" style={{ ...grid4, marginBottom: 18 }}>
      ...
      <Kpi rotulo="Ocorrências de risco moderado" valor={inteiro(resumo.itens_risco_moderado_atual)} detalhe="Soma mensal, pode repetir um mesmo insumo" tom="var(--risk-medio)" />
