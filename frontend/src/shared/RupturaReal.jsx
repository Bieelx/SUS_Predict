...
export function AlertasReais({ municipio, onOpenClara, deepLinkAlertaId }) {
  ...
  return <div className="rise">
    <header style={header}><div><h1 style={titulo}>Central de Alertas <span style={subtitulo}>— {dados.municipio.nome}, {dados.municipio.uf}</span></h1><p style={descricao}>Riscos de aquisição identificados na competência de referência da fonte.</p></div></header>
...
export function InsumosReais({ municipio }) {
  ...
  return <div className="rise">
    ...
    <Card style={{ overflow: 'hidden' }}><div style={{ padding: '18px 20px 5px' }}><SectionTitle>Itens da base de alertas na competência de referência</SectionTitle></div>{alertas.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th style={th}>Insumo</th><th style={th}>Risco</th><th style={{ ...th, textAlign: 'right' }}>Quantidade adquirida</th><th style={{ ...th, textAlign: 'right' }}>Valor adquirido</th><th style={{ ...th, textAlign: 'right' }}>Fornecedores</th></tr></thead><tbody>{alertas.map(item => <tr key={`${item.insumo_padronizado}-${item.unidade_fornecimento}`}><td style={{ ...td, fontWeight: 700 }}>{item.insumo_padronizado}<small style={{ display: 'block', color: 'var(--ink-400)', marginTop: 3 }}>{item.unidade_fornecimento || 'unidade não informada'}</small></td><td style={td}><Badge label={rotuloDado(item.faixa_risco_aquisicao)} color={riscoCor(item.faixa_risco_aquisicao)} /></td><td style={{ ...td, textAlign: 'right' }}>{inteiro(item.quantidade_adquirida)}</td><td style={{ ...td, textAlign: 'right' }}>{moeda(item.valor_adquirido)}</td><td style={{ ...td, textAlign: 'right' }}>{inteiro(item.total_fornecedores)}</td></tr>)}</tbody></table></div> : <Vazio texto={`Nenhum insumo monitorado retornado para ${dados.municipio.nome}.`} />}</Card>
  </div>;
}
