...
export function VisaoGeralReal({ municipio, onNavigate, estadual = false }) {
  ...
  return <div className="rise">
    ...
    <div className="responsive-grid-2" style={grid2}>
      ...
      <Card className="p-5"><SectionTitle>Índice composto analítico</SectionTitle>{risco ? <><div style={{ display: 'flex', alignItems: 'baseline', gap: 10, margin: '18px 0' }}><strong style={{ font: '800 42px JetBrains Mono, monospace', color: riscoCor(risco.faixa_risco) }}>{decimal(risco.indice_risco_regional)}</strong><Badge label={rotuloDado(risco.faixa_risco)} color={riscoCor(risco.faixa_risco)} /></div><p style={texto}>Escala fixa de 0 a 100. Não representa probabilidade de surto.</p>{[['Epidemiológico', risco.score_epidemiologico], ['Pressão hospitalar', risco.score_capacidade], ['Suprimento', risco.score_estoque_critico]].filter(([, value]) => value != null).map(([label, value]) => <div key={label} style={{ marginTop: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5 }}><span>{label}</span><strong>{decimal(value)}</strong></div><div style={track}><span style={{ ...fill, width: `${Math.min(100, n(value))}%` }} /></div></div>)}</> : <Vazio texto="Índice de risco não calculado para este território." />}</Card>
    </div>
...
export function AlertasReais({ municipio, onOpenClara, deepLinkAlertaId }) {
  const estado = useRuptura('12 Meses', municipio.ibge6);
  const dados = estado.dados;
  if (!dados) return <div className="rise"><header style={header}><div><h1 style={titulo}>Central de Alertas <span className="page-territory" style={subtitulo}>— {municipio.nome}, {municipio.uf}</span></h1><p style={descricao}>Riscos de aquisição identificados na competência de referência da fonte.</p></div></header><EstadoConsulta carregando={estado.carregando} erro={estado.erro} onRetry={estado.recarregar} /></div>;
  const alertas = dados.alertas || [];
    ...
    <Card style={{ overflow: 'hidden' }}>
      {alertas.length ? alertas.map((item, indice) => <article key={`${item.insumo_padronizado}-${item.unidade_fornecimento}`} style={{ padding: '16px 18px', borderBottom: indice < alertas.length - 1 ? '1px solid var(--ink-100)' : 0, background: selecionado === item ? 'var(--primary-soft)' : 'transparent' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 250 }}><div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 7 }}><Badge label={rotuloDado(item.faixa_risco_aquisicao)} color={riscoCor(item.faixa_risco_aquisicao)} /><span style={{ fontSize: 11, color: 'var(--ink-400)' }}>{rotuloDado(item.categoria_insumo)}</span></div><h2 style={{ fontSize: 15, margin: '0 0 6px' }}>{item.insumo_padronizado}</h2><p style={{ ...texto, margin: 0 }}>{item.mensagem_analitica}</p></div>
