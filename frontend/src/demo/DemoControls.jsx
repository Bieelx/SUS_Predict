  ...
  return <section className="demo-controls" aria-label="Demonstração histórica">
    <div className="demo-controls__row">
      ...
      <div className="demo-controls__actions">
        ...
        <label><span className="sr-only">Mês da demonstração</span><select value={replay.cutoff} onChange={event => onCorte(event.target.value)}>{cortes.map(mes => <option key={mes} value={mes}>{new Date(`${mes}-15T12:00:00`).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</option>)}</select></label>
