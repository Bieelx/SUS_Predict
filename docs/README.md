...
| [11 — Deploy em produção (Azure)](./11-deploy-azure.md) | Por que saímos do servidor de casa, opções de hospedagem descartadas, VM Azure for Students + Caddy, `.env` de produção, rotina de atualização e pendências (Telegram, Supabase, Ollama) — **no ar desde 10/09/2026** |
...
| Clara (pilar obrigatório) | Desenhado (texto automático: [docs/telas/01](./telas/01-visao-geral.md), Camada 2; painel de chat: [docs/telas/08](./telas/08-painel-clara.md); agente/backend: [docs/06](./06-agente-clara.md)) | Implementar: texto automático (Gemini) + painel de conversa com agente LangGraph, ferramentas de estoque/epidemiologia/alertas |
