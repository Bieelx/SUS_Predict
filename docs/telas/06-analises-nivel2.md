...
### Recorte de vacinação (decidido em reunião de 29/07/2026)

Cobertura vacinal deixa de ser módulo próprio e entra aqui como **cruzamento**: para
agravos com imunizante (sarampo, coqueluche, influenza, febre amarela, meningite), o gráfico
central ganha uma série de cobertura vacinal sobreposta à incidência — eixo duplo, mesma
janela temporal. A leitura que interessa é a correlação inversa ("cobertura caiu, incidência
subiu"), não a cobertura isolada.

- Só aparece quando o agravo selecionado tem imunizante correspondente; para dengue e
  tuberculose (sem/parcial) o recorte simplesmente não é oferecido
- Fonte: PNI (API própria do Ministério da Saúde, fora do FTP do DATASUS) — ainda não
  integrada, é coleta nova, não extensão de pipeline
- KPI extra quando ativo: cobertura atual × meta do PNI para o imunobiológico
