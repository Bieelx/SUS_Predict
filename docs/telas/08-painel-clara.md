...
grupo definiu que a Clara deve evoluir para um agente (ou sistema multi-agente) com
...
- A Clara é um ícone de chat flutuante, sempre visível, não um item de menu
...
- **Histórico** é acessível por um ícone no cabeçalho (relógio) que troca a área de
  ...
  nova.
- O histórico possui duas abas de origem: **App** e **Telegram**. As conversas não
  se misturam visualmente, mas continuam pertencendo ao mesmo usuário e podem ser
  abertas pelo painel web.
- No Telegram, 30 minutos de inatividade encerram a sessão corrente. A próxima
  mensagem cria outra conversa e preserva a anterior no histórico. O intervalo é
  configurável por `TELEGRAM_SESSION_TIMEOUT_MINUTES`.
...
| Resposta em texto | Backend do agente Clara (spec de arquitetura separado) |
...
| Painel fora de telas autenticadas (Login) | Não aplicável — Clara só faz sentido com usuário e município selecionados |
