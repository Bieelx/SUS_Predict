...
**Status: REVISAR** — mudança de abordagem decidida em reunião de grupo (29/07/2026)

## Revisão de escopo — sem rastreio de estoque em tempo real

Decisão do grupo: **não é viável rastrear estoque em tempo real.** Não existe fonte pública
com posição de estoque de secretaria, e o dado do DATASUS demora meses a cair — qualquer
"dias restantes" calculado sobre isso seria falso.

Nova abordagem (responsável: **Yasmin**): em vez de estoque × consumo, a métrica é
**quantidade de medicamentos comprados/dispensados × população do município** — um
indicador de suficiência relativa, não de ruptura iminente.

Consequências para o resto deste documento (as camadas abaixo descrevem o desenho antigo,
mantido aqui como referência até o novo ser detalhado):

- Camada 1 — "Itens críticos / em alerta" deixa de ser semáforo de ruptura; vira desvio do
  esperado por população (ex: compra per capita abaixo da mediana de municípios similares)
- Camada 2/3 — "dias restantes" e a curva de queda de 20 semanas **saem**; não há dado que
  os sustente
- CRUD de estoque e upload de planilha — perdem a razão de ser como fonte primária;
  reavaliar se ficam como entrada opcional ou saem do MVP
- **Defasagem do DATASUS vira elemento de tela obrigatório**: a competência mais recente
  disponível precisa aparecer explícita, mesmo padrão do selo de
  [06-analises-nivel2.md](./06-analises-nivel2.md) ("não é tempo real")

Pendente: definir a fonte exata de compras/dispensação (SIA? BPS? Hórus?) — ver
[07-pontos-em-aberto.md](./07-pontos-em-aberto.md).

---

## Desenho anterior (estoque × consumo) — mantido como referência
