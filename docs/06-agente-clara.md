...
[telas/08-painel-clara.md](./telas/08-painel-clara.md): como a Clara efetivamente
...
Clara deve responder perguntas livres do usuário com acesso a dado real (não só gerar
...
[03-arquitetura.md](./03-arquitetura.md) já antecipava "FastAPI + LangGraph (Clara)" na
...
- `gerar_etp(ibge6, item, alerta_id=None)` — **escrita.** Cria um registro em `etps`
  (nova tabela) com justificativa gerada a partir do estoque atual; se vinculado a um
  `alerta_id`, move esse alerta para `em_andamento` (mesma regra de
  [telas/03-central-alertas.md](./telas/03-central-alertas.md)). Implementada em
  `api/core/susbot_tools.py::gerar_etp`.

## Decisão 2.1 — Ferramentas de escrita exigem confirmação humana (07/08/2026)

Toda ferramenta que altera estado (hoje só `gerar_etp`, listada em
`susbot_tools.FERRAMENTAS_ESCRITA`) segue um fluxo de dois passos, nunca executa direto
a partir do plano do LLM:

1. O agente planeja normalmente. Se a ferramenta escolhida é de escrita, o backend
   **não a executa** — emite um evento SSE `confirmacao_pendente` com
   `{ferramenta, argumentos, resumo}` e encerra o stream sem chamar o LLM de resposta
   final (`ClaraAgent.stream_eventos`, em `api/core/susbot_agent.py`).
2. O frontend renderiza um botão de confirmação na própria bolha do chat. Ao confirmar,
   reenvia a pergunta com `{confirmar: {ferramenta, argumentos}}` no lugar de `pergunta`
   — o backend pula o planejamento e chama `ClaraAgent.stream_eventos_confirmado`
   direto, que executa a tool e só então gera a resposta final.

Ferramentas de leitura continuam executando direto no primeiro passo, sem esse ciclo.

## Decisão 2.2 — Rastreabilidade: artefato tipado, não só link de navegação (07/08/2026)

Além do evento `referencia` (link "ver em X"), o agente agora emite um evento
`artefato` com um payload tipado — `tabela` (estoque/alertas), `resumo` (estatísticas
de epidemiologia) ou `etp` (justificativa + `etp_id` após gerar) — construído a partir
do retorno bruto da tool (`_construir_artefato` em `susbot_agent.py`), não do texto
gerado pelo LLM. Isso é o "número citado é o número real" que faltava: o frontend
renderiza o artefato a partir do dado que realmente veio da ferramenta, então não há
como o texto do LLM divergir do número mostrado no artefato.
**Limite conhecido:** isso garante que o *artefato* não alucina — não impede que a
*prosa* do LLM cite um número diferente do artefato ao lado. Validação cruzada
texto↔artefato fica para uma v2.
...
O endpoint que recebe a pergunta do painel (`08-painel-clara.md`) entrega a resposta
...
```
Usuário digita pergunta no painel (08-painel-clara.md)
        ↓
...
08-painel-clara.md), incluindo referência de tela de origem se houver
