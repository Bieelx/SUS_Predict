# Briefing — Do Spec ao Código: Gaps de Backend do Chatbot (Clara)
...
([06-agente-clara.md](./06-agente-clara.md) e
[telas/08-painel-clara.md](./telas/08-painel-clara.md)) num plano de implementação
...
| Decisão | Onde está |
|---|---|
| Painel lateral à direita, campo livre, sem sugestões pré-definidas | [telas/08](./telas/08-painel-clara.md) |
| Conversas como threads (não linha do tempo única), histórico navegável, "Nova conversa" arquiva a atual | [telas/08](./telas/08-painel-clara.md) |
| Link "ver em X" navega na mesma aba, painel continua aberto | [telas/08](./telas/08-painel-clara.md) |
| Um agente, múltiplas ferramentas (não multi-agente com roteador) | [06](./06-agente-clara.md) |
| Ferramentas híbridas: parametrizadas (padrão) + SQL controlado via `sqlglot` (fallback) | [06](./06-agente-clara.md) |
| LangGraph + Gemini, entrega via streaming (SSE) | [06](./06-agente-clara.md) |
| Tabelas `estoque` e `alertas` nascem com **dado sintético** para demo | [06](./06-agente-clara.md) |
| Isolamento por município: **risco aceito nesta fase** (sem JWT completo ainda) | [06](./06-agente-clara.md) |
...
- `frontend/src/App.jsx` (linha ~1082 em diante) já tem um **mock** de Clara
  (`FloatingChat`, `askClara`) com respostas fixas/simuladas — é o protótipo que
